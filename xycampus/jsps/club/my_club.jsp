...
<html>
  ...
  <body>
    <ul class="pagination">
	    <li><a href="<c:url value='/ClubServlet?method=preList'/>" target="body">报名</a></li>
	    <li><a href="<c:url value='/ClubServlet?method=loadMyClubs'/>" target="body">我的社团纳新</a></li>
	</ul>
	<h2 align="center" style="color: rgba(0,127,127,20)">我的纳新</h2>
		...
		<table border="1" align="center" id="Main">
			...
			<c:forEach items="${requestScope.clubs }" var="club">
				<tr>
					<td>${club.community }</td>
			  		<td>${club.department }</td>
			  		<td>${club.uname }</td>
			  		<td>${fn:substring(club.start,0,10)}至${fn:substring(club.end,0,10)}</td>
			  		<td><a href="<c:url value='/ClubServlet?method=applyUsers&jid=${club.jid}'/>">查看报名人员</a></td>
				</tr>
			</c:forEach>
		</table>
